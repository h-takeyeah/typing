...
target "api" {
  ...
  tags = [for tag in target.docker-metadata-action.tags : "ghcr.io/h-takeyeah/typing-server:${tag}"]
...
target "app" {
  ...
  tags = [for tag in target.docker-metadata-action.tags : "ghcr.io/h-takeyeah/typing-app:${tag}"]
